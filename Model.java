...
class Model {
    ...
    void forNeighbors(Place place, Consumer<Place> action) {
        ...
        int j = place.col;
        if (i == 0 && j == 0) {
            action.accept(pl(i + 1, j));
            action.accept(pl(i, j + 1));
    ...
    final boolean solved() {
        int colorA = _cells[0][0];
        for (int i = 0; i < _cells.length; i++) {
            for (int j = 0; j < _cells[0].length; j++) {
                if (_cells[i][j] != colorA) {
                    return false;
                }
            }
        }
    ...
    HashSet<Place> findRegion(Place start, int color, HashSet<Place> result) {
        if (get(start) != color) {
            ...
            result.add(start);
            forNeighbors(start, (next) -> {
                if (get(next) == color && !result.contains(next)) {
                    findRegion(next, color, result);
                }
            });
        }
        return result;
    ...
    Set<Place> adjacentCells(int color) {
        ...
        for (Place i: _active) {
            forNeighbors(i, (next) -> {
                if (get(next) == color) {
                    adjacent.addAll(findRegion(next, color, new HashSet<>()));
                }
            });
        }
        return adjacent;
